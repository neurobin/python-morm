...
class Meta(metaclass=MetaType):
    ...
    * proxy (bool): Whether it is a proxy model. Defaults to False. This setting is inherited by child model. This is only to have different pythonic behavior of a model. Proxy models can not define new fields and they do not have separate db table but share the same db table as their parents.
