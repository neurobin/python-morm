...
class Meta(metaclass=MetaType):
    ...
    * `indexes` (*Dict[str, Dict]*): Define named composite indexes. Each key is an index name. Each value is a dict with `cols` (field names as a list/tuple, a single name, or a comma-separated string) and `indexes` (index methods as a comma-separated string or list/tuple: `btree`, `hash`, `gin`, `gist`, `spgist`, `brin`, optional `method:opclass`). Example: `{'my_index': {'cols': ['id', 'userID'], 'indexes': 'hash,btree'}}`.
