...
## Special Model Meta attributes `f` and `fs`:

You can access field names from `ModelClass.Meta.f` and SELECT SQL fragments from `ModelClass.Meta.fs`.

`Meta.f` allows a spell-safe way to write field names for INSERT/UPDATE dict keys (persisted fields) or SQL expressions (`ExprField`). `Meta.fs` returns SELECT list fragments for both persisted and expression fields.
...
# custom SELECT: f'SELECT {fs.name}, {fs.profession} FROM ...'
```

If you misspell a name, you will get `AttributeError`.
...
* `f`: Access field names (persisted fields) or SQL expressions (`ExprField`).
* `fs`: Access SELECT list SQL fragments (`"col" AS "col"` or `(expr) AS "name"`).

## ExprField (calculated SELECT expressions)

Use `ExprField` for values computed in SQL at read time. They are not stored in the database and are excluded from INSERT, UPDATE, and migrations.
...
Default SELECT queries (`qfilter`, `get`) include ExprFields automatically. Use `Meta.fs` or `ModelQuery.fs` for hand-built SELECT lists:
...
await db.q(Order).q(
    f'SELECT {fs.price}, {fs.qty}, {fs.total} FROM {qh.db_table} WHERE ({f.total}) > ${qh.c}',
    100,
).fetch()
```

- `Meta.f` / `qh.f` — field names, quoted columns, or parenthesized expressions (for WHERE / ORDER BY).
